# Build RAG Reference Files & Vector Store

Complete guide for the two RAG build scripts:

- **`build_rag_reference_files.py`** — generates the shipped, embedding-free Markdown
  reference files under `starboard_core/rag/knowledge/domains/` (the default runtime path).
- **`build_rag_vector_store.py`** — builds the opt-in ANN/SQLite vector store (legacy
  pipeline, not part of the default runtime path).

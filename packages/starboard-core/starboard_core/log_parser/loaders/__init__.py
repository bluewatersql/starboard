...
from starboard_core.log_parser.parsing_models.exceptions import (
...
class FileChunkStreamWrapper:
    ...
    def __init__(
        ...
        maximum_file_size: int = ArchiveExtractionThresholds.size,
    ):
        assert chunks
        ...
        self._buffer_position: int = (
            0  # Index of how far into _buffer/_buffer_memview we have read
        )
    ...
    def _should_read_more(self, size) -> bool:
        return (
            not size or self._num_bytes_left_in_buffer() < size
    ...
    def read(self, size=-1) -> memoryview:
        ...
        self._buffer_position = (
            ...
            if (size and not self._consumed)
...
FileExtractionResult = TypeVar(
    "FileExtractionResult", bound=tuple[Path, FileStreamIterator]
)


class BaseDataLoader(abc.ABC):
    """
    Simple base class for data loaders that provides load/load_many interface.
    Replaces aiodataloader dependency with a simpler synchronous approach.
    """
    ...
    def batch_load_fn(self, keys: list):
        """
        Subclasses should implement this method to load data for multiple keys.
        Should return a list with one result per key in the same order.
        """
        pass

    def load(self, key):
        """Load a single item"""
        return self.batch_load_fn([key])[0]
...
class AbstractFileDataLoader(BaseDataLoader, abc.ABC):
    """
    ...
    def __init__(self, extraction_thresholds: ArchiveExtractionThresholds = None):
        super().__init__()
        self._extraction_thresholds: ArchiveExtractionThresholds = (
            extraction_thresholds or ArchiveExtractionThresholds()
        )
    ...
    def read_tgz_archive(
        self, archive: tarfile.TarFile
    ) -> Iterator[FileExtractionResult]:
        """
        ...
        for tarinfo in archive:
            ...
            wrapped_bytes = FileChunkStreamWrapper(
                archive.extractfile(tarinfo), size_left
            )
    ...
    def read_zip_archive(self, raw_archive_stream) -> Iterator[FileExtractionResult]:
        ...
        for fname, fsize, chunks in stream_unzip(
            raw_archive_stream, chunk_size=self._ARCHIVE_CHUNK_SIZE
        ):
            num_files += 1
            if num_files > self._extraction_thresholds.entries:
                raise ArchiveTooManyEntriesError()
    ...
    def extract(
        self, filepath: Path, file_stream: BufferedIOBase = None
    ) -> Iterator[FileExtractionResult]:
        """
        ...
        match filepath.suffixes:
            case [".tgz"] | [".tar", ".gz"]:
                logger.debug(f"Unpacking gzipped tarball: {filepath}")
            ...
            case [".zip"]:
                logger.debug(
                    f"Unpacking .zip archive: {filepath} from stream: {file_stream}"
                )
                if file_stream:
                    ...
                    # unzipping streams that are not seekable (as is the case when given an e.g. HTTP Stream).
            ...
            case [*suffixes] if len(suffixes) > 0 and suffixes[-1] == ".gz":
                logger.debug(f"Processing gzipped file: {filepath}")
            ...
            case [".json"] | [".log"] | []:
                logger.debug(f"Yielding raw file: {filepath}")
                match file_stream:
                    case None:
            ...
            case _:
                raise LogSubmissionException(
                    f"Unknown file format {''.join(filepath.suffixes)}"
                )
    ...
    def batch_load_fn(self, keys: list[str]) -> list[Iterator[FileExtractionResult]]:

...
def parse_line(line: str, pattern: str, var_names: list[str]) -> dict[str, str]:
...
class DagModel:
    def __init__(self) -> None:
        self.stage_dict: dict[int, str] = {}  # <k="stage_id", v="trans_id" >
        self.alias_dict: dict[str, list[int]] = defaultdict(
            list
        )  # <k="trans_id", v=["stage_id"]>
        self.parents_dag_dict: dict[int, list[int]] = defaultdict(
            list
        )  # <k="parent_stage", v=["child_stage"]>
        self.rdd_stage_dict: dict[int, list[int]] = defaultdict(
            list
        )  # <k="RDD_id", v=["Stage_IDs"]  >
        self.stage_rdd_dict: dict[int, list[int]] = defaultdict(
            list
        )  # <k="stage_id", v=["RDD_ids"]    >
        self.rdd_parent_dict: dict[int, list[int]] = defaultdict(
            list
        )  # <k="RDD_id", v=["RDD_parents"]>
        self.rdd_persist_dict: dict[
            int, dict[int, bool]
        ] = {}  # <k="stage_id", v="persist?"     >
        self.broadcast_stages: list[int] = []  # stages that have BroadcastExchange

    # Extract the DAG from the Spark log SparkListenerJobStart and SparkListenerStageStart
    def parse_dag(self, data: dict[str, Any]) -> None:
        # Create the stage alias dictionaries
        # NOTE: SparkListenerJobStart will always be before SparkListenerStageStart
        if "Stage Infos" in data:
            ...
            for stage in stages:
                ...
                if stage_id not in self.rdd_persist_dict:
                ...
                for x in stage["RDD Info"]:
                    ...
                    # If there is a duplicate instance of this RDD, consider it cached if any version specifies caching
                        ...
                        self.rdd_persist_dict[stage_id][rdd_id] = (
                            persisted or previous_persist
                        )

                    # Keep track of RDDs for this stage -- the keys provide a branchless set
                    rdd_dict.add(rdd_id)

                    self.rdd_parent_dict[rdd_id] += (
                        rdd_parents  # Keep track of RDD-level parents
                    )
    ...
    def decipher_dag(self) -> None:
        ...
        for child in phantom_dependencies:
            phantoms = phantom_dependencies[child]
            for phantom in phantoms:
                self.parents_dag_dict[child].remove(phantom)

    def check_persisted_rdd_dependencies(self, child: int) -> None:
        stages = self.parents_dag_dict.keys()
        potential_parents = []
        ...
        for rdd_id in self.stage_rdd_dict[child]:
            ...
            for parent in set(self.rdd_parent_dict[rdd_id]):
                ...
                parent_stages = [
                    ...
                    if rdd_id in self.rdd_persist_dict[x]
                    and self.rdd_persist_dict[x][rdd_id]
                ]
        ...
        parent_list: list[int] = [x for x in set(potential_parents) if x in stages]
        if parent_list:
            parent_list = [min(parent_list)]
        self.parents_dag_dict[child] = parent_list

    def add_broadcast_dependencies(self, stdoutpath: str | None) -> None:
        ...
        for parent_stage in reversed(self.broadcast_stages):
            ...
            for stage_id in stage_list[idx:]:
                ...
                if (parent_stage not in self.parents_dag_dict[stage_id]) and (
                    not any(
                        s in stage_list[idx:] for s in self.parents_dag_dict[stage_id]
                    )
                ):
                    self.parents_dag_dict[stage_id].append(parent_stage)

    def parse_broadcast(self, lines: list[str]) -> None:
        # Parse lines for broadcast (other than from DAGScheduler) and add implicit dependencies
        # patterns to parse
        broadcast_pattern = (
            ".*?SparkContext: Created broadcast [0-9]* from .*? at (.*?):[0-9]*"
        )
    ...
    def parse_stdout(self, stdoutpath: str) -> None:

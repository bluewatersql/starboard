...
class EventDataValidation:
    ...
    def validate(self) -> None:
        ...
        if not self.app.finish_time:
            self.message += (
                MSGS.MISSING_EVENT_GENERIC_MESSAGE
                + "'Application / Stage / SQL Completion'. "
            )
    ...
    def validate_job_events(self) -> None:
        """
    ...
    def validate_stage_events(self) -> None:
        """
        ...
        for _jid, job in self.app.jobs.items():
        ...
        if len(miss_stage_completes) > 0:
            self.message += (
                f"{MSGS.MISSING_EVENT_STAGE_COMPLETE}{miss_stage_completes}.  "
            )

"""Pytest configuration for starboard-log-parser package.
...
if fixtures_path.exists():
    spec = importlib.util.spec_from_file_location("logparser_fixtures", fixtures_path)
    if spec and spec.loader:
        fixtures_module = importlib.util.module_from_spec(spec)
        sys.modules["logparser_fixtures"] = fixtures_module
        spec.loader.exec_module(fixtures_module)

        # Import fixtures into this namespace so pytest can discover them
        sample_event_log_path = fixtures_module.sample_event_log_path

"""Databricks Lakebase memory store implementation."""
...
class DatabricksLakebaseMemoryStore(PostgresMemoryStore):
    """

    Databricks Lakebase-backed long-term memory store.

    Extends PostgresMemoryStore with Databricks OAuth token management:
    - Automatic token refresh every 50 minutes (tokens expire after 1 hour)
    - Databricks SDK integration for credential generation
    - PostgreSQL-compatible via asyncpg with pgvector support
    ...
    Architecture:
        ...
        the existing PostgreSQL memory store and adds automatic OAuth token refresh.
    ...
    Example:
        ...
        store = DatabricksLakebaseMemoryStore(config)
        await store.connect()

        # Use like a regular PostgreSQL memory store
        episode = Episode(...)
    ...
    def __init__(self, config: DatabricksLakebaseConfig):
        """
        Initialize Databricks Lakebase memory store.
        ...
        # PostgresMemoryStore expects connection_string in __init__
        super().__init__("")
    ...
    async def connect(self) -> None:
        ...
        """
        logger.debug(
            "databricks_lakebase_memory_connect_start",
        ...
        logger.debug(
            "databricks_lakebase_memory_connect_complete",
            instance=self._config.instance_name,
    ...
    async def close(self) -> None:
        ...
        logger.debug(
            "databricks_lakebase_memory_close_start",
            instance=self._config.instance_name,
        )
        ...
        logger.debug(
            "databricks_lakebase_memory_close_complete",
            instance=self._config.instance_name,
        )

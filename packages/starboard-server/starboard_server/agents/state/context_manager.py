...
if TYPE_CHECKING:
    from starboard_server.agents.multi_agent_manager import (
...
class ContextManager:
    ...
    def _reconstruct_context(self, existing: Any) -> SharedAgentContext:
        ...
        """
        from starboard_server.agents.state.shared_context import (
            AgentTransition,
            SharedAgentContext,
        )
    ...
    def _convert_messages(self, messages: list[Any]) -> list[Message]:
        ...
        for msg in messages:
            if isinstance(msg, dict):
                ...
                #  starboard_server.agents.state.Message does not)
                converted.append(
                    Message(
                        role=msg.role,
                        content=msg.content,

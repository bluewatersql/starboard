...
SELECT
  ...
  t1.run_as,
  t1.total_dbus,
  ROUND(t1.total_dbus / t1.runs, 2) AS avg_dbus_per_run,
  t1.last_seen_date
FROM dbu_per_job t1
...
run_states AS (
  SELECT workspace_id, job_id, run_id, FIRST(result_state, TRUE) AS result_state
...
job_dbus AS (
  SELECT workspace_id, usage_metadata.job_id AS job_id, usage_metadata.job_run_id AS run_id,
    ROUND(SUM(usage_quantity), 2) AS run_dbus
  FROM system.billing.usage
  WHERE usage_metadata.job_id IS NOT NULL AND billing_origin_product = 'JOBS'
...
SELECT
  ...
  ROUND(MAX(jrd.duration_mins) / NULLIF(MIN(jrd.duration_mins), 0), 2) AS max_min_ratio,
  ROUND(AVG(jd.run_dbus), 2) AS avg_dbus,
  ROUND(SUM(jd.run_dbus), 2) AS total_dbus,
  ROUND(AVG(jd.run_dbus / NULLIF(jrd.duration_mins, 0)), 4) AS avg_dbus_per_minute
FROM job_run_durations jrd
LEFT JOIN latest_jobs j USING (workspace_id, job_id)
LEFT JOIN job_dbus jd ON jrd.workspace_id = jd.workspace_id AND jrd.job_id = jd.job_id AND jrd.run_id = jd.run_id
...
run_summary AS (
  SELECT workspace_id, job_id, run_id,
    COUNT(*) - 1 AS repairs,
    FIRST(result_state, TRUE) AS final_result_state
  FROM system.lakeflow.job_run_timeline
  WHERE period_start_time >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
    AND result_state IS NOT NULL
  GROUP BY workspace_id, job_id, run_id
...
dbu_by_state AS (
  SELECT rs.workspace_id, rs.job_id,
...
SELECT
  ...
  js.retried_runs,
  js.total_repairs,
  ROUND(js.failures * 1.0 / NULLIF(js.total_runs, 0) * 100, 1) AS failure_rate_pct,
  ROUND(js.total_repairs * 1.0 / NULLIF(js.total_runs, 0), 2) AS avg_repairs_per_run,
  ds.total_dbus,
  ds.failure_dbus,
  ds.retry_dbus,
  ROUND((ds.failure_dbus + ds.retry_dbus) / NULLIF(ds.total_dbus, 0) * 100, 1) AS wasted_dbu_pct
FROM job_stats js
LEFT JOIN latest_jobs j USING (workspace_id, job_id)
...
SELECT
  ...
  ROUND(COUNT(DISTINCT CASE WHEN result_state = 'FAILED' THEN run_id END) * 100.0 /
    NULLIF(COUNT(DISTINCT run_id), 0), 1) AS failure_rate_pct
FROM system.lakeflow.job_run_timeline
WHERE period_start_time >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  AND result_state IS NOT NULL
...
SELECT
  ...
  ROUND(SUM(CASE WHEN t.result_state = 'FAILED' THEN 1 ELSE 0 END) * 100.0 /
...
LEFT JOIN latest_jobs j USING (workspace_id, job_id)
WHERE t.period_start_time >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  AND t.result_state IS NOT NULL
GROUP BY j.name, t.workspace_id, t.job_id, t.task_key
HAVING failures > 0
...
update_stats AS (
  SELECT
    workspace_id, pipeline_id, update_id, update_type,
...
SELECT
  ...
  ROUND(SUM(CASE WHEN us.result_state = 'FAILED' THEN 1 ELSE 0 END) * 100.0 /
    NULLIF(COUNT(DISTINCT us.update_id), 0), 1) AS failure_rate_pct
FROM update_stats us
LEFT JOIN latest_pipelines p USING (workspace_id, pipeline_id)
GROUP BY p.name, us.workspace_id, us.pipeline_id, p.pipeline_type, us.update_type

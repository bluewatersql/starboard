...
SELECT
  workspace_id,
  COALESCE(usage_metadata.app_name, usage_metadata.endpoint_name) AS app_name,
  sku_name,
  ...
  DATEDIFF(DAY, MAX(usage_end_time), CURRENT_TIMESTAMP()) AS days_since_last_activity
FROM system.billing.usage
WHERE usage_date >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  AND (billing_origin_product = 'APPS' OR sku_name LIKE '%DATABRICKS_APPS%' OR sku_name LIKE '%APPS_SERVERLESS%')
GROUP BY ALL
ORDER BY dbus DESC
"""
...
WITH app_billing AS (
  SELECT workspace_id,
    COALESCE(usage_metadata.app_name, usage_metadata.endpoint_name) AS app_identifier,
    ROUND(SUM(usage_quantity), 2) AS total_dbus
  FROM system.billing.usage
  WHERE (billing_origin_product = 'APPS' OR sku_name LIKE '%DATABRICKS_APPS%')
    AND usage_date >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  GROUP BY workspace_id, COALESCE(usage_metadata.app_name, usage_metadata.endpoint_name)
),
app_access AS (
  SELECT workspace_id,
...
SELECT
  ab.workspace_id, ab.app_identifier, ab.total_dbus,
  COALESCE(aa.access_events, 0) AS user_access_events,
  COALESCE(aa.distinct_users, 0) AS distinct_users,
  aa.last_user_access,
  DATEDIFF(DAY, aa.last_user_access, CURRENT_TIMESTAMP()) AS days_since_accessed,
  CASE
    WHEN aa.access_events IS NULL THEN 'ZOMBIE: Never accessed'
    WHEN DATEDIFF(DAY, aa.last_user_access, CURRENT_TIMESTAMP()) > 14 THEN 'IDLE: No access > 14 days'
    WHEN DATEDIFF(DAY, aa.last_user_access, CURRENT_TIMESTAMP()) > 7 THEN 'STALE: No access > 7 days'
    ELSE 'ACTIVE'
  END AS status
FROM app_billing ab
LEFT JOIN app_access aa ON ab.workspace_id = aa.workspace_id AND ab.app_identifier = aa.app_identifier
...
SELECT
  workspace_id,
  COALESCE(usage_metadata.database_instance_id, usage_metadata.endpoint_name, 'unknown') AS lakebase_instance,
  sku_name,
  CASE
    WHEN sku_name LIKE '%COMPUTE%' OR sku_name LIKE '%CU%' THEN 'Compute (CUs)'
    WHEN sku_name LIKE '%STORAGE%' THEN 'Storage'
    WHEN sku_name LIKE '%IO%' OR sku_name LIKE '%READ%' OR sku_name LIKE '%WRITE%' THEN 'I/O Operations'
    ELSE 'Other'
  END AS dimension,
  usage_unit,
...
FROM system.billing.usage
WHERE usage_date >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  AND (billing_origin_product = 'LAKEBASE' OR sku_name LIKE '%LAKEBASE%' OR sku_name LIKE '%MANAGED_POSTGRES%')
GROUP BY ALL
...
SELECT
  ...
  CASE
    WHEN sku_name LIKE '%VECTOR_SEARCH_INDEX_CREATION%' OR sku_name LIKE '%VECTOR_SEARCH_SYNC%' THEN 'Index Creation / Sync'
    WHEN sku_name LIKE '%VECTOR_SEARCH_SERVING%' OR sku_name LIKE '%VECTOR_SEARCH_QUERY%' THEN 'Index Serving / Queries'
...
FROM system.billing.usage
WHERE usage_date >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  AND (billing_origin_product = 'VECTOR_SEARCH' OR sku_name LIKE '%VECTOR_SEARCH%')
GROUP BY ALL
ORDER BY year_month DESC, usage_quantity DESC
"""
...
FROM system.billing.usage
WHERE usage_date >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  AND (billing_origin_product = 'DATA_SHARING' OR sku_name LIKE '%DELTA_SHARING%' OR sku_name LIKE '%DATA_SHARING%')
GROUP BY ALL
ORDER BY year_month DESC, usage_quantity DESC
"""
...
SELECT
  workspace_id,
  CONCAT_WS('.', usage_metadata.uc_table_catalog, usage_metadata.uc_table_schema, usage_metadata.uc_table_name) AS monitored_table,
  sku_name,
  DATE_TRUNC('MONTH', usage_date) AS year_month,
...
FROM system.billing.usage
WHERE usage_date >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  AND (billing_origin_product = 'LAKEHOUSE_MONITORING' OR sku_name LIKE '%LAKEHOUSE_MONITORING%')
GROUP BY ALL
...
SELECT
  ...
  DATE_TRUNC('MONTH', usage_date) AS year_month,
  CASE
    WHEN sku_name LIKE '%SERVERLESS%' AND billing_origin_product = 'SQL' THEN 'Serverless SQL'
...
GROUP BY workspace_id, DATE_TRUNC('MONTH', usage_date),
  CASE
    WHEN sku_name LIKE '%SERVERLESS%' AND billing_origin_product = 'SQL' THEN 'Serverless SQL'
    WHEN billing_origin_product = 'SQL' THEN 'Classic SQL Warehouse'
    WHEN billing_origin_product = 'ALL_PURPOSE' THEN 'All-Purpose (interactive)'
...
task_stats AS (
  ...
  WHERE period_start_time >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
    AND result_state IS NOT NULL
  GROUP BY workspace_id, job_id, run_id, task_key
)
SELECT
  j.name AS job_name,
  ts.job_id, ts.workspace_id, ts.task_key,
  ...
  ROUND(SUM(ts.failure_count) * 100.0 / NULLIF(SUM(ts.execution_count), 0), 1) AS failure_rate_pct
FROM task_stats ts
LEFT JOIN latest_jobs j USING (workspace_id, job_id)
GROUP BY j.name, ts.job_id, ts.workspace_id, ts.task_key
...
P_WF02_SQL = """\
SELECT
  workspace_id, job_id, run_id, task_key,
  COUNT(*) AS iteration_count,
  ROUND(SUM(CAST(period_end_time - period_start_time AS LONG)) / 60.0, 2) AS total_cpu_mins,
  ROUND(MAX(CAST(period_end_time - period_start_time AS LONG)) / 60.0, 2) AS max_iteration_mins,
  ROUND(MIN(CAST(period_end_time - period_start_time AS LONG)) / 60.0, 2) AS min_iteration_mins,
  ROUND(CAST(MAX(period_end_time) - MIN(period_start_time) AS LONG) / 60.0, 2) AS wall_clock_mins,
  ROUND(CAST(MAX(period_end_time) - MIN(period_start_time) AS LONG) /
    NULLIF(SUM(CAST(period_end_time - period_start_time AS LONG)), 0), 4) AS parallelism_ratio,
...
SELECT
  ...
  ROUND(PERCENTILE(total_duration_ms / 1000.0, 0.95), 2) AS p95_duration_secs,
  SUM(CASE WHEN execution_status = 'FAILED' THEN 1 ELSE 0 END) AS failed_queries,
  SUM(CASE WHEN from_result_cache = 'true' THEN 1 ELSE 0 END) AS cache_hits,
...
WHERE start_time >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  AND compute.warehouse_id IS NOT NULL
  AND (executed_by LIKE '%genie%' OR executed_by LIKE '%ai-bi%' OR executed_by LIKE '%aibi%' OR client_application LIKE '%Genie%')
GROUP BY workspace_id, compute.warehouse_id, executed_by, DATE(start_time)
...
SELECT
  ...
  DATEDIFF(DAY, MAX(event_time), CURRENT_TIMESTAMP()) AS days_since_accessed
FROM system.access.audit
WHERE event_time >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  AND service_name IN ('dashboards', 'sql/dashboards', 'lakeview')
  AND action_name IN ('getDashboard', 'viewDashboard', 'runDashboard', 'refreshDashboard')
GROUP BY workspace_id, request_params['dashboardId'], request_params['dashboardName'], DATE(event_time)

...
billing_summary AS (
  ...
  WHERE usage_metadata.cluster_id IS NOT NULL
    AND (sku_name LIKE '%ALL_PURPOSE_COMPUTE%' OR sku_name LIKE '%JOBS_COMPUTE%')
    AND usage_start_time >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  GROUP BY usage_metadata.cluster_id
)
SELECT
  c.workspace_id, c.cluster_id, c.cluster_name,
  CASE WHEN c.min_autoscale_workers IS NOT NULL THEN 'Autoscaling' ELSE 'Fixed Size' END AS cluster_type,
  c.cluster_source,
  c.min_autoscale_workers, c.max_autoscale_workers, c.worker_count AS fixed_workers,
  c.worker_node_type, c.driver_node_type,
  c.auto_termination_minutes, c.owned_by,
  nm.worker_node_count, nm.avg_cpu_pct, nm.peak_cpu_pct, nm.avg_mem_pct, nm.peak_mem_pct,
  nm.avg_cpu_wait_pct,
  bs.total_dbus, bs.active_days, bs.last_used,
...
idle_time AS (
  ...
  WHERE (cpu_user_percent + cpu_system_percent) < 5
    ...
    AND (driver IS NULL OR driver = FALSE)
    AND start_time >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  GROUP BY cluster_id
),
billing_summary AS (
  SELECT usage_metadata.cluster_id AS cluster_id,
    ROUND(SUM(usage_quantity), 2) AS total_dbus
  FROM system.billing.usage
  WHERE sku_name LIKE '%ALL_PURPOSE_COMPUTE%'
    AND usage_start_time >= CURRENT_DATE() - INTERVAL {lookback_days} DAYS
  GROUP BY usage_metadata.cluster_id
)
SELECT
  c.workspace_id, c.cluster_id, c.cluster_name,
  c.cluster_source, c.owned_by,
  c.auto_termination_minutes,
  CASE WHEN c.auto_termination_minutes IS NULL THEN 'NO AUTO-TERMINATE'
       WHEN c.auto_termination_minutes > 120 THEN 'HIGH (>2hr)'
       WHEN c.auto_termination_minutes > 60 THEN 'MEDIUM (1-2hr)'
       ELSE 'LOW (<1hr)' END AS termination_risk,
  it.idle_minutes,
  bs.total_dbus,
  ROUND(it.idle_minutes * 1.0 / NULLIF(30 * 24 * 60, 0) * COALESCE(bs.total_dbus, 0), 2) AS est_idle_dbus
FROM latest_clusters c
...
C_C03_SQL = """\
WITH scale_events AS (
  SELECT warehouse_id,
    DATE(event_time) AS event_date, HOUR(event_time) AS event_hour,
...
query_perf AS (
  SELECT
    ...
    DATE(start_time) AS query_date, HOUR(start_time) AS query_hour,
    COUNT(*) AS total_queries,
    ROUND(AVG(execution_duration_ms / 1000.0), 2) AS avg_execution_secs,
    ROUND(PERCENTILE(execution_duration_ms / 1000.0, 0.95), 2) AS p95_execution_secs,
...
SELECT
  qp.warehouse_id, qp.query_date, qp.query_hour,
  qp.total_queries, qp.avg_execution_secs, qp.p95_execution_secs,
  qp.avg_queue_secs, qp.max_queue_secs, qp.queries_queued_30s_plus,
  qp.failed_queries,
  se.scale_up_events, se.scale_down_events, se.peak_cluster_count
FROM query_perf qp

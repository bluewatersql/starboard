...
Uses starboard-core log parser to parse Spark event logs and extract
...
class SparkEventLogExtractor:
    ...
    Uses starboard-core log parser for parsing, then extracts:
    - Failed jobs with reasons
    - Slow stages (duration outliers)
    - Executor removals/OOM
    - Task failure counts
    ...
    async def extract(
        ...
        """
        # Parse using starboard-core log parser
        lines_iter = (
            json.loads(line)
    ...
    def _extract_evidence(self, app: Any) -> SparkDiagnosticEvidence:
        ...
        Args:
            app: Parsed ApplicationModel from starboard-core log parser

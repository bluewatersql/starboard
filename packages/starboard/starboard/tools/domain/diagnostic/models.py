...
# PrimarySymptom, ExplorationSummary and _PATTERN_TO_SYMPTOM were re-homed to the
# dep-light ``starboard_x`` namespace in ``starboard-core`` (Phase-1 B2). They are
# re-imported here so this module remains the full diagnostic-model surface while
# keeping a single source of truth (no divergence between the two packages).
...
# ``PrimarySymptom`` is imported from ``starboard_x.diagnostic.models`` (see top
# of this module). Its definition was re-homed there in Phase-1 B2.
...
# ``_PATTERN_TO_SYMPTOM`` and ``ExplorationSummary`` are imported from
# ``starboard_x.diagnostic.models`` (see top of this module). Their definitions
# were re-homed there in Phase-1 B2.

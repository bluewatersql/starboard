...
(``starboard.agents.events``) and the domain conversation models
(``starboard.domain.conversation.models``). The old import path
``starboard.api.event_converter`` re-exports these symbols for backward
compatibility.
...
from starboard.domain.conversation.models import ChatEvent, EventType
...
def validate_event_coverage() -> tuple[bool, list[str]]:
    ...
    from starboard.agents.events import StreamingEvent

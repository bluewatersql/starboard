...
``starboard.domain.conversation.models``; this module simply re-exports
them. The streaming-event converter is an adapter concern and is re-exported
from ``starboard.adapters.conversation.event_converter``.

The previous architectural-debt inversion (finding F-3-1a-001), where this
module imported from ``starboard.api``, has been resolved: the api layer
...
if TYPE_CHECKING:
    ...
    # the adapter imports starboard.agents.events, whose package __init__
